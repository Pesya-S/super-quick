...
export class ViewShopComponent implements OnInit {
  ...
  r:RequrestResponse ;
  name:string="red";
  constructor(private db:DbService,public dialog:MatDialog,public functions:FunctionsService)
   {  
    this.arrColor.push("white");
    this.arrColor.push("grey");
    this.arrColor.push("pink");
    this.arrColor.push("white");
    let s:Shop=db.getShop();
    let dref:MatDialogRef<ProgresBarComponent>;
    dref=  this.functions.openDialog(this.dialog,30,10,ProgresBarComponent,null,false);
    this.db.GetMap(s).subscribe(
      x=>{
      this.r=  <RequrestResponse>x;
       if(this.r.Result==true)
       {this.mat=<number[][]>(this.r.Data);
         this.openDialog("הנה החנות שלך",this.r.Message); }
       else
       this.openDialog("שגיאה",this.r.Message); 
     dref.close(); }); 
 }
  
 ngOnInit() { 

  //this.db.getWithParams(db.getShop().)
 }
